...
type Space = M.HashMap (Int, Int, Int, Int) Bool
...
readInput list = go list 0 M.empty
  where
    ...
    go2 [] _ _ acc = acc
    go2 (c : cs) x y acc = go2 cs (x + 1) y (if c == '#' then M.insert (x, y, 0, 0) True acc else acc)

dimensions :: Space -> ((Int, Int), (Int, Int), (Int, Int), (Int, Int))
dimensions s =
  let ks = M.keys s
      xs = map (\(a, _, _, _) -> a) ks
      ys = map (\(_, b, _, _) -> b) ks
      zs = map (\(_, _, c, _) -> c) ks
      us = map (\(_, _, _, d) -> d) ks
   in ((minimum xs, maximum xs), (minimum ys, maximum ys), (minimum zs, maximum zs), (minimum us, maximum us))
...
step :: Space -> Space
step s =
  let ((x1, x2), (y1, y2), (z1, z2), (u1, u2)) = dimensions s
      ixs = [(x, y, z, u) | x <- [(x1 - 1) .. (x2 + 1)], y <- [(y1 - 1) .. (y2 + 1)], z <- [(z1 - 1) .. (z2 + 1)], u <- [(u1 -1) .. (u2 + 1)]]
   in foldl' (checkNeighbours s) M.empty ixs

neighbours = [(x, y, z, u) | x <- [-1 .. 1], y <- [-1 .. 1], z <- [-1 .. 1], u <- [-1 .. 1], (x, y, z, u) /= (0, 0, 0, 0)]

checkNeighbours :: Space -> Space -> (Int, Int, Int, Int) -> Space
checkNeighbours oldSpace acc p@(x, y, z, u) =
  let values = map (\(a, b, c, d) -> (M.lookup (a + x, b + y, c + z, d + u) oldSpace)) neighbours
