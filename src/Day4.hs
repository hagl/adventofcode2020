...
isValid s = let
    attrs = splitOn " " s
    req = sort ["byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid"]
    keys = sort $ map (head . splitOn ":") attrs
  in isSubsequenceOf req keys

isValid2 :: String -> Bool
isValid2 s = let
    attrs = splitOn " " s
    req = sort ["byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid"]
    keys = sort $ map head $ filter isValidAttr $ map (splitOn ":") attrs
  in isSubsequenceOf req keys
  where
    isValidAttr [k, v] = isValidAttribute k v

isValidAttribute :: String -> String -> Bool
isValidAttribute "ecl" value = elem value ["amb", "blu", "brn", "gry", "grn", "hzl", "oth"]
isValidAttribute "hcl" value = value =~ [re|^#[0-9a-f]{6}$|]
isValidAttribute "pid" value = value =~ [re|^[0-9]{9}$|]
isValidAttribute "byr" value = isBetween value 1920 2002
isValidAttribute "iyr" value = isBetween value 2010 2020
isValidAttribute "eyr" value = isBetween value 2020 2030
isValidAttribute "hgt" value = 
  let 
    matches = (value =~ [re|^([0-9]+)(in|cm)$|]) :: [[String]]
  in case matches of 
    [_:numberString:unit:[]] -> unit == "in" && isBetween numberString 59 76 || unit == "cm" && isBetween numberString 150 193
    _ -> False

isValidAttribute _ _ = True



isBetween :: String -> Int -> Int -> Bool
isBetween s min max = case readMaybe s of
  Nothing -> False
  Just x -> min <= x && x <= max
